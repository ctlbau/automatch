...
def fetch_drivers(province_id):
	...
	WHERE
		D.province_id = :province_id) AS match1
	LEFT JOIN (
	SELECT
		D.kendra_id AS driver_id, D.name, DV.vehicle_id
	FROM
		Drivers D
		JOIN DriversVehiclesExchangeLocations DV ON D.kendra_id = DV.driver_id) AS match2 ON match1.vehicle_id = match2.vehicle_id AND match1.driver_id != match2.driver_id
	GROUP BY
		match1.driver_id,
		match1.driver,
		match1.street,
